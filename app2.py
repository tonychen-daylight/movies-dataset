...
if (
    ...
    st.dataframe(st.session_state.trip_history, width=1000, height=400, column_config={"trip_number":st.column_config.NumberColumn(format="%d")})
    ...
    st.dataframe(st.session_state.trips_matched, width=1000, height=400, column_config={
    ...
    if container2.button("Show Summary", type="secondary") or (len(st.session_state.trips_matched) > 0):
        ...
        sel_loop_name = 'EWR-DAL-ONT-Tuesday-Friday-Sunday'
        avg_value = group_df[sel_loop_name].values[0]
        #st.write(avg_value)
        ...
        event = st.dataframe(loopList_deduped, width=1000, height=400,  use_container_width=True,
            hide_index=True,
        ...
        if (len(selected)> 0):
            ...
            df = pd.DataFrame(loopList_deduped).iloc[selected]
            #st.write(df)
            ...
            st.dataframe(loopDetail, width=1200, height=200, column_config={
                "TripNumber1":st.column_config.NumberColumn(format="%d"),
                "TripNumber2":st.column_config.NumberColumn(format="%d"),
                "TripNumber3":st.column_config.NumberColumn(format="%d"),
                "TotalOldCost":st.column_config.NumberColumn(format="$%.02f"),
